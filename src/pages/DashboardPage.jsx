...
// dummy summary (untuk demo)
function makeSensorSummary() {
    return {
...
// dummy time-series agregat (untuk chart dashboard)
function makeOverallSeries(sensorKey) {
    const now = Date.now();
    return Array.from({ length: 60 }).map((_, i) => {
        const t = new Date(now - (59 - i) * 60_000);

        let val = 0;
        if (sensorKey === "temp") val = 28 + Math.sin(i / 6) * 1.1;
        if (sensorKey === "vib") val = 0.45 + Math.abs(Math.cos(i / 8)) * 0.75;
        if (sensorKey === "press") val = 40 + Math.sin(i / 5) * 9;
...
export default function DashboardPage() {
    ...
    const summary = useMemo(() => makeSensorSummary(), []);

    // overall (agregat) charts
    const overallTemp = useMemo(() => makeOverallSeries("temp"), []);
    const overallPress = useMemo(() => makeOverallSeries("press"), []);
    const overallVib = useMemo(() => makeOverallSeries("vib"), []);
    ...
    return (
        <div className="min-h-screen bg-[#e9eef3] p-4 sm:p-6">
            <div className="w-full mx-auto space-y-4 sm:space-y-6">
                ...
                {/* OVERALL TREND (mengisi space kosong bawah tanpa menduplikasi DetailPage) */}
                <div className="neo-surface p-4 sm:p-6 rounded-2xl">
                    <div className="flex items-end justify-between gap-3">
                        <div>
                    ...
                    <div className="mt-4 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
                        <MiniTrend title="Temperature" unit="°C" data={overallTemp} />
                        <MiniTrend title="Pressure" unit="N" data={overallPress} />
                        <MiniTrend title="Vibration" unit="A" data={overallVib} />
                    </div>
                </div>
            </div>
        </div>
